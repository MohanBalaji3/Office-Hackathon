...
export function AnalysisSummary({ result }: AnalysisSummaryProps) {
  ...
  const safePassRate = summary?.passRate ?? (summary?.total > 0 ? (summary.passed / summary.total) * 100 : 0);
  const safeDuration = summary?.duration ?? '0.00s';
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      <Card className="border-l-4 border-l-primary">
        ...
        <CardContent>
          <div className="text-3xl font-bold text-primary">{safePassRate.toFixed(1)}%</div>
