...
export function JiraSettingsDialog({ onCredentialsSaved }: JiraSettingsDialogProps) {
  ...
  const testConnection = async () => {
    ...
    setIsTesting(true);
    try {
      // Test JIRA connection through backend to avoid CORS issues
      const response = await fetch('http://localhost:4000/api/test-jira-connection', {
        method: 'POST',
        headers: {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Could not connect to JIRA. Please check your credentials.",
        variant: "destructive",
      });
    } finally {
      setIsTesting(false);
